...
- `data/nuclear.dip` owns a reusable `nuclear_simulation` schema, so DIPL rejects invalid scenario values before C++ runs—such as a non-positive duration or a mass supplied where a time is required.
