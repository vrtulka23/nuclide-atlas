...
`data/nuclear.dip` is a registry, not a monolithic data dump. Its `$source` records point at individual files and import their `isotope` tables beneath `nuclear.isotopes`. This makes diffs and provenance reviews local to one record.
